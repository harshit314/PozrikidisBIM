...
double dt = 0.005, tFinal = 40.0, dtMin = 0.05;
...
void setV(vector<BIMobjects> & spheroids, ThreeDVector *posDx, ThreeDVector *dTheta)
{
    ...
    for (int iter = 0; iter < 250; iter++)
    {
        ...
        for (int iObj = 0; iObj < nspheroids; iObj++)
        {
            ...
            if(myRank==0 && iter%5==0)    cout<<"iteration no:"<<iter+1<<"; AvgError: "<<totalError/spheroids[0].nCoordFlat<<endl;
...
int main(int argc, char **argv)
{
    ...
    spheroidTemplate.scale(0.35, 1.0, 1.0);
    spheroidTemplate.rotate(ThreeDVector(0.0, 0.0, 1.0), -0.341007);

    vector<BIMobjects> spheroids; 
    spheroids.push_back(spheroidTemplate);

    //spheroidTemplate.translate(ThreeDVector(0.750724, -0.330193, 0.0));
    spheroidTemplate.translate(ThreeDVector(0.750724*1.7, -0.330193*1.7, 0.0));
    
    spheroidTemplate.rotate(ThreeDVector(0.0, 0.0, 1.0), 0.341007 - 0.794286);
    spheroids.push_back(spheroidTemplate);

  //  spheroidTemplate.translate(ThreeDVector(0.0, 2.0, 0.0));
  //  spheroidTemplate.rotate(ThreeDVector(0.0, 0.0, 1.0), M_PI/6.0);
  //  spheroids.push_back(spheroidTemplate);
    
  //  spheroidTemplate.translate(ThreeDVector(1.9, 0.0, 0.0));
  ...
  //  spheroids.push_back(spheroidTemplate);
    ...
    dOrient.push_back(ThreeDVector(cos(-0.341007), sin(-0.341007), 0.0));
    dOrient.push_back(ThreeDVector(cos(-0.794286), sin(-0.794286), 0.0));
  //  dOrient.push_back(ThreeDVector(cos(1.0*M_PI/24.0), sin(1.0*M_PI/24.0), 0.0));
  //  dOrient.push_back(ThreeDVector(cos(11.0*M_PI/24.0), -sin(11.0*M_PI/24.0), 0.0));
   ...
   // if(myRank==0)   { spheroids[0].storeElemDat(); }
    ...
    if(myRank==0)
    {
        outPos.open("./OutputData/posSpheroidPozDrift.txt", ios::out);
        outPos.precision(nPrecision);
        outVel.open("./OutputData/velSpheroidPozDrift.txt", ios::out);
        outVel.precision(nPrecision);
        outTym.open("./OutputData/timePozDrift.txt", ios::out);
    ...
    {    
		...
		cout<<spheroids[i].uRB.x[0]<<'\t'<<spheroids[i].uRB.x[1]<<endl;
                cout<<spheroids[i].omega.x[2]<<endl;
	    }
        }
